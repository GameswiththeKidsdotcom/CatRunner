...
- **Targets:** CatRunnerTests (75) + CatRunnerUITests (8).
- **Baseline:** **83 tests** (75 + 8; 2 skipped when monetization not configured — J4a/J4c; pass count for regression comparison). When checking "test currently running" or "test completed", look for this command (or equivalent `xcodebuild test` with scheme CatRunner) in the terminals folder.
- **Manual simulator:** When testing manually in simulator, always Run from Xcode (⌘R) or use the run-simulator script so the simulator uses the latest build. See [app-store-readiness.md](../docs/app-store-readiness.md#manual-simulator-testing).

### Admin / other (npm)

- **Command:** `npm run test:full` (if present in the repo). Used for admin panel or other non-iOS tests.
- **Baseline:** Separate from iOS. Tester should treat iOS test results and npm test results as **distinct baselines** — e.g. "iOS 83 tests pass" (or with 2 skips) vs "npm test:full N/M pass". When checking "test currently running" or "test completed", include both: (1) iOS `xcodebuild test` runs, (2) `npm run test:full` (or similar) for admin.
...
- Check the **terminals folder** (e.g. `~/.cursor/projects/.../terminals/` or the workspace terminals) for any terminal that has a **current command**. Consider **both**:
  1. **iOS:** `xcodebuild test -scheme CatRunner ...` (or equivalent with scheme CatRunner); baseline 83 tests (75 CatRunnerTests + 8 CatRunnerUITests; 2 skipped when monetization not configured).
...
1. **Identify which baseline** the results belong to: **iOS** (xcodebuild test, 83 tests) or **admin/npm** (e.g. `npm run test:full`). Compare against that baseline only.
