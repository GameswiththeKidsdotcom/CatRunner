...
**Playwright viewport keys (for future web version):** `iPhone SE (3rd gen)`, `iPhone 15`, `iPhone 14`, `iPhone 15 Pro Max` (see [Agents/ui-test.md](../../../../Agents/ui-test.md) device matrix). Resolve from `playwright.devices` or deviceDescriptorsSource.json when adding web E2E.
...
- **Simulator matrix:** Run app on each of the 5 simulators above; confirm player visible, lanes visible, obstacles moving top→bottom. Document pass/fail per device.
...
- **ui-test agent:** Refine alignment criteria and 5-iPhone matrix per [Agents/ui-test.md](../../../../Agents/ui-test.md); add Playwright viewport keys if web version exists.

This file is the P002 viewport deliverable for native iOS (Lane A).
